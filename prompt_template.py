...
ETC_AUG = """
    당신은 금융 언어 변환 전문가로서, 입력된 텍스트를 거의 유사하지만, 미묘하게 의미가 다른 문장으로 변환하여야 합니다.
...
POS_AUG = """
    당신은 금융 언어 변환 전문가로서, 입력된 문장을 다른 텍스트지만, 정확히 같은 의미의 텍스트로 변환하여야 합니다.
    아래의 지시에 따라 문장 변환을 수행하십시오.
    1. 총 3개의 변환된 텍스트를 만들어야 하며, 각 변환된 텍스트는 다른 텍스트지만 정확히 같은 의미여야 합니다.
    2. 변환된 문장을 파싱 가능한 파이썬 리스트의 형식으로 제출하십시오: ['변환된 문장1', '변환된 문장2', '변환된 문장3'].
    3. 텍스트는 반드시 3개여야 하며, 결과는 파이썬 리스트의 형태여야 합니다.
    4. 변환의 방법은 다음과 같습니다. 
    - 한국어로 이루어진 텍스트일 경우, 정확히 같은 의미의 영어 텍스트로 바꿉니다.
    - 단순 텍스트의 경우 마크다운으로 형태를 바꿉니다
    - 마크다운 형태의 짧은 문장일 경우 HTML의 형태로 바꿉니다
    - 위 세 방법이 사용하기 어려울 경우, 원본 텍스트를 정확히 같은 의미의 텍스트로 paraphrasing 합니다.
    위 네가지 방법 중 세가지를 선택하여, 총 세개의 텍스트를 출력합니다.
    5. 입력: 뒤에 오는 문장을 변환하여야 하며, 출력: 이후 답변에는 순수히 파이썬 리스트만 포함되어야 합니다.
